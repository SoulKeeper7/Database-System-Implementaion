...
Type Function :: RecursivelyBuild (struct FuncOperator *parseTree, Schema &mySchema) {
	...
	// in this case, we have either a literal value or a variable value, so do a push
		...
		pch = strtok(parseTree->leftOperand->value, ".");
		while (pch != NULL)
		{
			//cout << pch;
			prev = pch;
		...
		if (parseTree->leftOperand->code == NAME) 
		{
			...
			int myType = mySchema.FindType (prev);
			puts(strcat(parseTree->leftOperand->value, "."));
			puts(strcat(parseTree->leftOperand->value, x.c_str()));
			//string x = pch;
			//puts(strcat(parseTree->leftOperand->value, x.c_str()));
