...
extern "C" struct AndList *boolean;
...
void q0 (){
	...
	char *cnf = "SELECT  s.suppkey 	FROM  supplier AS s, partsupp AS ps WHERE(s.s_suppkey = ps.ps_suppkey) 		GROUP BY s.s_suppkey";
	...
	cnf = "SELECT  s.suppkey 	FROM  supplier AS s, partsupp AS ps WHERE(s_suppkey>1000)	GROUP BY s.s_suppkey";
	yy_scan_string(cnf);
	yyparse();
	double dummy = s1.Estimate(boolean, relName, 1);
	s1.Apply(boolean, relName, 2);
...
void q1 ()
{
	...
	char *cnf = "SELECT l.l_orderkey, l.l_partkey, l.l_suppkey  	FROM lineitem AS l 	WHERE(l.l_returnflag = 'R') AND (l.l_discount < 0.04 OR l.l_shipmode = 'MAIL')";

		
	//char *cnf = "(l_returnflag = 'R') AND (l_discount < 0.04 OR l_shipmode = 'MAIL')";
	//char *cnf = "(l_discount < 0.04 OR l_shipmode = 'MAIL') AND (l_returnflag = 'R')";
	...
	cout << result;
	cout<<"Your estimation Result  " <<round(result);
...
void q2 (){
	...
	char *cnf = "SELECT o.o_custkey 	FROM c_custkey AS c,o_custkey AS o	WHERE (c_custkey = o_custkey)";
	...
	//cnf = " (c_nationkey = c_nationkey)"; 
	cnf = "SELECT c_nationkey 	FROM c_custkey AS c,n_nationkey AS n WHERE (c_nationkey = n_nationkey)";
	yy_scan_string(cnf);
	yyparse();
...
void q4 (){
	...
	yy_scan_string(cnf);
	yyparse();
	cout << sizeof(relName) / sizeof(relName[0]);
	s.Apply(boolean, relName, 2);
	...
	double result = s.Estimate(boolean, relName, 5);
	cout << result;
	if(fabs(result-3200)>0.1)
		cout<<"error in estimating Q4\n";

	s.Apply(boolean, relName, 5);
